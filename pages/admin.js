...
export default function AdminPanel() {
  ...
  // إرسال الرد من الإدارة للمستخدم
  const sendReply = () => {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-6 font-cairo" dir="rtl">
      ...
      <div className="flex gap-4 mb-8">
        <button onClick={() => setActiveTab('products')} className={`px-6 py-3 rounded-2xl font-black ${activeTab === 'products' ? 'bg-yellow-400 text-black shadow-lg shadow-yellow-400/20' : 'bg-zinc-900 text-zinc-500'}`}>الأجهزة المعروضة ({products.length})</button>
        <button onClick={() => setActiveTab('messages')} className={`px-6 py-3 rounded-2xl font-black ${activeTab === 'messages' ? 'bg-yellow-400 text-black shadow-lg shadow-yellow-400/20' : 'bg-zinc-900 text-zinc-500'}`}>طلبات الدعم ({messages.length})</button>
      </div>
