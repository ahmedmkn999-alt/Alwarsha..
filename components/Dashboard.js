          ...
          <>
                        ...
                        <div className="absolute top-[50px] left-4 right-4 bg-zinc-900 rounded-2xl mt-2 p-2 shadow-2xl z-[60] border border-zinc-800 max-h-60 overflow-y-auto animate-slideDown">
                            ...
                            {categories.map(cat => (
                                <button key={cat.id} className="w-full text-right p-3 text-sm hover:bg-zinc-800 rounded-xl transition-colors font-bold text-white flex justify-between items-center" onClick={() => {setSearchTerm(cat.name); setShowSearchSuggestions(false);}}>
                                    <span>🔍 {cat.name}</span><span className="text-[10px] text-zinc-600">قسم</span>
                                </button>
                            ))}
                        </div>
                    </>
                  ...
                  <div className="flex gap-4 overflow-x-auto no-scrollbar px-4 pb-2">
                    ...
                    {categories.map(cat => (
                      <div key={cat.id} onClick={() => setSelectedCategory(cat.name)} className={`flex-shrink-0 w-24 aspect-[4/6] rounded-[1.5rem] relative overflow-hidden cursor-pointer border-2 transition-all ${selectedCategory === cat.name ? 'border-yellow-400 scale-105 shadow-xl' : 'border-transparent opacity-80'}`}>
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end justify-center p-3 text-center"><span className="text-white text-[10px] font-black leading-tight">{cat.name}</span></div>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {filtered.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-6">
                  {filtered.map(item => (
                    <ProductCard key={item.id} item={item} onViewImage={setViewImage} onChat={(it) => setMessageModal({ show: true, receiverId: it.sellerId, receiverName: it.sellerName })} isOwner={item.sellerId === user.uid} onDelete={deleteProduct} />
                  ))}
                </div>
                ...
                uniqueConversations.sort((a,b) => new Date(b.date) - new Date(a.date)).map(chat => (
                    <div key={chat.id} className="flex gap-2 items-center relative select-none">
                        ...
                        <div onContextMenu={(e) => { e.preventDefault(); }} onTouchStart={() => handleTouchStart(chat.fromId === user.uid ? chat.toId : chat.fromId, chat.fromName)} onTouchEnd={handleTouchEnd} onClick={() => { if (!readChats.includes(chat.fromId === user.uid ? chat.toId : chat.fromId)) setReadChats([...readChats, chat.fromId === user.uid ? chat.toId : chat.fromId]); setMessageModal({ show: true, receiverId: chat.fromId === user.uid ? chat.toId : chat.fromId, receiverName: chat.fromName }); }} className={`flex-1 bg-white p-6 rounded-[2rem] border flex items-center gap-5 cursor-pointer hover:border-yellow-400 transition-all shadow-sm ${pinnedChats.includes(chat.fromId === user.uid ? chat.toId : chat.fromId) ? 'border-yellow-400 ring-2 ring-yellow-100 bg-yellow-50' : ''}`}>
                            ...
                            <div className="flex-1 text-right"><h4 className="font-black text-zinc-900">{chat.fromName}</h4><p className="text-xs text-zinc-400 line-clamp-1 mt-1">{chat.text || (chat.image ? "📸 صورة" : "🎤 رسالة صوتية")}</p></div>
                        </div>
                    </div>
                ))
            }
        ...
        {activeTab === 'support' && (
          <div className="max-w-md mx-auto space-y-6">
            ...
            <div className="bg-white p-4 rounded-[2.5rem] border shadow-inner h-[300px] overflow-y-auto flex flex-col gap-3 no-scrollbar">
                {myMessages.filter(m => m.fromId === 'Admin' || m.toId === 'Admin').map((msg, i) => (
                    <div key={i} className={`p-3 rounded-2xl text-xs font-bold max-w-[80%] ${msg.fromId === user.uid ? 'bg-yellow-400 self-end text-black' : 'bg-zinc-100 self-start text-zinc-800'}`}>{msg.text || " رسالة"}</div>
                ))}
            </div>
          ...
          <div className="max-w-xl mx-auto text-right">
            ...
            <div className="grid grid-cols-1 gap-4">
                {products.filter(p => p.sellerId === user.uid).map(item => (
                    <ProductCard key={item.id} item={item} onViewImage={setViewImage} onChat={() => {}} isOwner={true} onDelete={deleteProduct} />
                ))}
            </div>
          </div>
        )}
