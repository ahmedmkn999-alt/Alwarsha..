    ...
    <div className="min-h-screen bg-zinc-50 pb-24 font-cairo" dir="rtl">
      ...
      <header className="bg-zinc-950 text-white shadow-xl sticky top-0 z-50 border-b-2 border-yellow-400">
        ...
        <div className="container mx-auto px-4 pb-2">
            <input className="w-full bg-zinc-900 border-none rounded-2xl p-2 text-xs text-white outline-none focus:ring-1 focus:ring-yellow-400 font-bold" placeholder="ابحث في فولت..." onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </header>

      {/* Tabs: شريط التنقل الجديد (الرئيسية، جديد، مستعمل) */}
      ...
      <main className="container mx-auto p-4 md:p-8 animate-fadeIn">
        ...
        {activeTab === 'profile' && (
          <div className="max-w-xl mx-auto">
            ...
            <h3 className="text-lg font-black mb-6 pr-3 border-r-4 border-yellow-400">إعلاناتي</h3>
        ...
        {/* عرض المنتجات (الرئيسية / جديد / مستعمل) */}
        {(activeTab === 'home' || activeTab === 'new' || activeTab === 'used') && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filtered.map(item => (
              <div key={item.id} className="bg-white rounded-[2rem] shadow-sm border border-gray-50 overflow-hidden group hover:shadow-2xl transition-all duration-500">
            ...
            {filtered.length === 0 && <div className="col-span-full text-center py-20 text-zinc-400 font-bold">لا توجد منتجات حالياً</div>}
          </div>
        )}
        ...
        {activeTab === 'support' && (
          <div className="max-w-md mx-auto bg-white p-8 rounded-[2.5rem] border border-gray-100 text-center">
            <h2 className="text-xl font-black mb-4">الدعم الفني 🎧</h2>
        ...
        <div className="fixed inset-0 bg-black/95 z-[110] flex items-center justify-center backdrop-blur-md p-0 md:p-6">
          <div className="bg-white w-full max-w-lg h-full md:h-[85vh] md:rounded-[3rem] flex flex-col shadow-2xl relative">
            ...
            <div className="p-5 bg-white border-t md:rounded-b-[3rem] flex gap-2 items-center relative">
               ...
               <input className="flex-1 bg-zinc-100 p-4 rounded-2xl outline-none font-bold text-xs" placeholder={isRecording ? (isCancelled ? "اترك للحذف" : "اسحب يمين للإلغاء ➡️") : "اكتب رسالتك..."} value={msgText} onChange={(e) => setMsgText(e.target.value)} disabled={isRecording} />
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 z-[120] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg p-8 rounded-[2.5rem] relative overflow-y-auto max-h-[90vh]">
             ...
             <h2 className="text-xl font-black mb-6">نشر إعلان جديد 🚀</h2>
